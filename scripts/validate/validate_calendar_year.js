...
function main() {
  ...
  const dir = args.dir || path.join(outRoot, String(year), "calendar_year");

  const referencePath = args.reference || path.join(dir, `${year}.moon_calendar.v1.json`);
  const deckPath = args.deck || path.join(dir, `${year}.calendar_year.deck.v1.json`);
  const exportPath = args.export || path.join(dir, `${year}.calendar_year.export.v1.json`);
  ...
  console.log(`[validate] ok: calendar_year ${year}`);
}
