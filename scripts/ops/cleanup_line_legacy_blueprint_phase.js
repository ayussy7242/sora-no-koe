...
function inspectDoc(doc) {
  ...
  if (!blueprintPhase) {
    return {
      ...
      notes: "blueprint_phase is empty; cleanup should run only after migration",
    };
  }
  ...
  if (blueprintPhase !== state) {
    return {
      ...
      eligible: false,
      action: "manual_review",
      notes: "state and blueprint_phase differ",
    };
  }
  ...
  return {
    ...
    eligible: true,
    action: "clear_state",
    notes: "safe to clear legacy blueprint phase from state",
  };
}
...
async function main() {
      ...
      await db.collection("line_users").doc(row.line_user_id).set(
        {
          ...
          meta: {
            last_event_type: "legacy_blueprint_phase_cleanup",
  ...
  console.log("LEGACY LINE BLUEPRINT PHASE CLEANUP");
  console.log(JSON.stringify(summary, null, 2));
  if (!inspected.length) {
    console.log("No migrated legacy blueprint phase records found.");
    return;
  }
