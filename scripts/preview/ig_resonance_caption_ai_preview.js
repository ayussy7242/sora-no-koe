...
const { buildIgResonancePrompt, generateIgResonanceText } = require("../../src/usecases/channels/instagram/ai/resonance");
...
async function main() {
  ...
  // IG共鳴captionは story.outputs.ig.source.resonance_aspect を参照するので注入
  const rep = pickResonanceDailyRepresentative({ story, dateLocal, resonanceMode, stepMinutes: 60 });
  const candidate = rep?.ok ? rep.candidate : null;
  story.outputs = story.outputs || {};
  story.outputs.ig = story.outputs.ig || {};
  story.outputs.ig.source = story.outputs.ig.source || {};
  story.outputs.ig.source.resonance_aspect = candidate;

  const variant = "caption";
  const prompt = buildIgResonancePrompt({ story, dict, variant });
  ...
  if (!openai.apiKey) {
    console.error("[ig_resonance_caption_ai_preview] OPENAI_API_KEY missing (use --prompt-only to just print the prompt)");
    process.exit(1);
  }
  ...
  const outDir = path.join(process.cwd(), "tmp", "ig", "resonance_caption_ai", dateLocal);
  fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, `ig_resonance_caption_${dateLocal}_${String(asOfISO).replace(/[:+]/g, "_")}.txt`);
  ...
  console.log("");
  console.log(`[ig_resonance_caption_ai_preview] saved: ${outPath}`);
}
