...
async function handleJobsWorker(req, res, deps = {}) {
  ...
  try {
    ...
    // 既存があっても、min.bodies / engine.houses が欠けてたら補完する
    const hasMinBodies = existing?.min?.bodies && typeof existing.min.bodies === "object";
    const hasEngineHouses = existing?.engine?.houses && typeof existing.engine.houses === "object";
    ...
    const patch = {
      ...
      needs_compute: false,
      computed_at: cacheSnap.exists ? (existing.computed_at || admin.firestore.FieldValue.serverTimestamp()) : admin.firestore.FieldValue.serverTimestamp(),
      updated_at: admin.firestore.FieldValue.serverTimestamp(),
    };
