...
/**
 * jobs/worker.js — Unified STABLE (v2025.12+ FINAL)
 *
 * - jobs_natal_calc queued を 1件だけ安全に取得して処理（transaction + lease）
 * - stale running を復旧（running → queued）
 * - attempts 上限
 * - natal_cache を idempotent に「作る or 必要な時だけ再計算」
 *
 * 重要:
 * - natal_cache 再利用は「birth_hash が一致」した場合のみ
 * - ただし houses が欠損/壊れてたら再計算で埋める
 */
