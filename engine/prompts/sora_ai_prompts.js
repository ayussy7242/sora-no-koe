...
- 星座は「出方」。
- 度数は「濃度・段階」。
- 配置は“意味の断定”ではなく、“反応の入り方/残り方”として記述する。
- 読み物として成立する温度を許可する（冷たくしない）。

【書き方（許可すること）】
- 天体名・星座名・度数・軸は本文に出してよい（商品仕様）。
- 1セクションは「読み応え」が出る密度でよい（目安 200〜380字）。
- 2段落構成を基本にしてよい：
  1段落目＝その天体の“役割×出方×度数段階”を短く芯で置く
  2段落目＝日常の場にどう現れやすいか（比喩/手触り/残り方）で広げる
- 比喩はOK（ただし煽らない・断定しない・指示しない）。
- 「圧」という語は避けてよい（重さ/密度/手触り/留まり方/張り などで溶かす）。
 - bodies.items は INPUT.natal.bodies と同じ数・同じ順序で出す（key は sun..pluto）。
 - angles.items は asc / mc / ic / dc の4件をこの順序で出す。

【長さ】
- 各 text は最低180字（目安220〜380字）。
- 1行で終わらない。

【段落】
- 改行を1つ入れて2段落にする。
- 1段落目：役割×出方×度数段階を芯で置く（2〜3文）
- 2段落目：日常の場での残り方を手触りで広げる（2〜3文）
...
- 比率から「反応の入り方」「動きの質」を記述する。
- 強い/弱いではなく、入り方・残り方で書く（冷たい集計コメントにしない）。

■ bodies
- 役割（惑星）× 出方（星座）を中心に書く。
- 度数は“段階”として自然に触れてよい（初期/中盤/後半/最終…など）。
- 断定しない範囲で、現れ方を意味づけしてよい（読み物の厚み）。

■ chiron / lilith
- 心理診断にせず、「触れたときの構造」「止まり方/濃度」で書く。
