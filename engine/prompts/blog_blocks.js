...
以下のFACTSから「天体配置」の本文を1段落で書く。
出力は日本語本文のみ（HTMLなし）。
文字数は150〜250字を目安にする。
機能の説明（抽象）→サインでの発揮→位置関係→緊張/支援の順で書く。
占い化しない。未来断定/指示/救済/運命固定/絶対語は禁止。
主語は構造。
`.trim(),
...
出力は日本語本文のみ（HTMLなし）。
文字数は150〜220字を目安にする。
角度の構造→サイン要素の組み合わせ→接近/離脱→圧の残り方の順で書く。
占い化しない。未来断定/指示/救済/運命固定/絶対語は禁止。
主語は構造。
`.trim(),
...
出力は日本語本文のみ（HTMLなし）。
文字数は400〜600字を目安にする。
優勢元素の物性→欠損の影→区分との組み合わせ→圧の質感の順で書く。
占い化しない。未来断定/指示/救済/運命固定/絶対語は禁止。
主語は構造。
`.trim(),
