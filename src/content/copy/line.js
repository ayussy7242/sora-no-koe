"use strict";

/**
 * copy/line.js — LINE専用コピー（唯一の正本）
 * - ここはUI/会話の世界（ボタン/案内/登録フロー）
 * - 占い化しない：未来断定しない / 行動指示しない / 良し悪し言わない
 */

const LINE_COPY = Object.freeze({
  BOT_NAME_FALLBACK: "ソラのこえ。｜宇宙の配置を可視化する",

  WELCOME: ({ nickname = "あなた", botName } = {}) => {
    return (
      "🌌 ソラのこえ はじめまして🩷\n\n" +
      "友だち追加ありがとう🛸✨\n\n" +
      "ここは「占い」ではなく、\n" +
      "今日の星の配置をそのまま置く場所\n\n" +
      "解釈は、あなたのものです\n\n" +
      "まずは「はじめる」と送ってください\n" +
      "→ 生年月日を登録すると、\n" +
      "星の設計図（Blueprint）をお渡しします\n\n" +
      "登録後は、毎朝8時に\n" +
      "「今日のそら」を配信していきます🌙\n\n" +
      "わからないところは「不明」でOK"
    );
  },

  HELP:
    "使い方🌌\n\n" +
    "■ できること\n" +
    "・「きょう」：\n" +
    "　あなたの星 × 今日のそら（主要共鳴）\n\n" +
    "・「わたし」：\n" +
    "　あなたの星の一覧（ASC/MC ふくむ）\n\n" +
    "・「そら」：\n" +
    "　今日のソラの星の配置（全体）をみる。\n\n" +
    "■ 登録する（個人版）\n" +
    "・「はじめる」\n\n" +
    "登録で聞くもの（わからなければ「不明」でOK）\n" +
    "1) 生年月日（例：1990-07-24 / 19900724）\n" +
    "2) 出生時刻（例：12:18）\n" +
    "3) 出生地（例：札幌 / Yokohama / Tono, Iwate）",
  FALLBACK:
    "できることはこの3つ🌌\n" +
    "・「きょう」\n" +
    "・「わたし」\n" +
    "・「そら」\n" +
    "個人版を使うなら「はじめる」🕊️",

  TEST_ACK:
    "受け取ったよ🌌\n" +
    "「きょう」 / 「そら」 / 「わたし」\n" +
    "登録は「はじめる」🕊️",

  CANCELLED:
    "OK、登録はいったん中断したよ🌌\n" +
    "またやるなら「はじめる」\n" +
    "今日は「そら」/「きょう」で見られるよ🕊️",

  RESET_DONE:
    "ネイタル登録をリセットしたよ🌌\n" +
    "もう一回やるなら「はじめる」🛸",

  // ---- natal flow（natal.js側で使う） ----
  START_NATAL:
    "個人版（あなたの星）を登録するよ🌌\n\n" +
    "まずは【生年月日】を送ってね🛸\n" +
    "例：1990-07-24（または 19900724）\n\n" +
    "わからなければ「不明」でもOK。\n" +
    "やめたくなったら「やめる」でOKだよ✨",

  ASK_BIRTH_TIME:
    "次に【出生時刻】を送ってね🕰️\n" +
    "例：12:18（24h）\n\n" +
    "時刻が不明だと ASC/MC が簡略になるよ。\n" +
    "わからなければ「不明」でもOK。",

  ASK_BIRTH_PLACE:
    "最後に【出生地】を送ってね📍\n" +
    "例：東京 / 神奈川県横浜市 / Shimizu, Hokkaido / Tono, Iwate\n\n" +
    "市区町村だけでもOK（例：岩手県遠野市）\n" +
    "（場所は緯度経度に変換して計算に使うよ）\n" +
    "わからなければ「不明」でもOK。",

  ERR_BIRTHDATE:
    "生年月日が読み取れなかった🙏\n" +
    "例：1990-07-24（または 19900724）\n" +
    "わからなければ「不明」でもOK。",

  ERR_BIRTHTIME:
    "出生時刻が読み取れなかった🙏\n" +
    "例：12:18\n" +
    "わからなければ「不明」でもOK。",

  NATAL_DONE:
    "✨ Blueprintが完成しました\n\n" +
    "あなたの星の設計図をお届けします🌌\n\n" +
    "あわせて使えるコマンド\n" +
    "・「わたし」 → あなたの星の一覧\n" +
    "・「きょう」 → 今日の接点\n" +
    "・「そら」 → 今日の空の配置",
  NATAL_RECEIVED:
    "🌌 登録ありがとう 💙\n" +
    "データを受け取りました🪐\n\n" +
    "これからあなたの星の設計図（Blueprint）を作成します\n" +
    "完成まで少し時間がかかります\n\n" +
    "できあがったら、このLINEにお届けします✨️\n\n" +
    "準備中に見れるもの\n" +
    "・「そら」 → 今日の空の配置\n" +
    "・「使い方」 → ヘルプ",
  BLUEPRINT_RESEND_NOTICE:
    "🌌 お待たせしました\n\n" +
    "お届けの流れに遅れが出ていました\n\n" +
    "星の設計図を、\n" +
    "ここにお送りします⭐️\n\n" +
    "すでに受け取っている場合は、\n" +
    "そのまま閉じてください✨️",
  PERSONAL_PREPARING:
    "🌌 いま設計図を準備しています\n\n" +
    "登録データは受け取れています🪐\n" +
    "ネイタル計算と Blueprint の生成に少し時間がかかります\n\n" +
    "完成したらこのLINEにお届けします✨️\n\n" +
    "その間は\n" +
    "・「そら」→ 今日の空の配置\n" +
    "・「使い方」→ ヘルプ\n" +
    "が見れます",
  NATAL_PARTIAL_SKIP:
    "受け取ったよ🌌\n\n" +
    "いまの情報でも動くよ。\n" +
    "出生地が不明だと、一部が簡略になることがある。\n\n" +
    "あとで整えたくなったら「はじめる」で上書きできるよ🕊️",

  // ---- natal flow（place geocode fail / job / cache） ----
  NATAL_PLACE_GEOCODE_PREFIX:
    "受け取ったよ🌌\n" +
    "出生地を座標に変換してるよ…📍\n",

  NATAL_PLACE_GEOCODE_FAIL_REASON:
    "場所がうまく特定できなかったみたい🕊️\n" +
    "・市区町村だけ（例：遠野市）\n" +
    "・英語表記（例：Tono, Iwate）\n" +
    "・座標（例：39.33, 141.53）\n\n" +
    "書き方を変えるか、「不明」でも進めるよ。",

  NATAL_PLACE_GEOCODE_NO_GEOCODER_REASON:
    "いま地図変換の処理が止まってるみたい。\n" +
    "ここはシステム側の問題だよ🙏\n\n" +
    "この項目はスキップして、先に進むね🕊️",

  // ---- natal list ----
  NATAL_LIST_NEED_LINK:
    "まだ登録がないみたい🌌\n" +
    "まず「はじめる」で登録してね🕊️",

  NATAL_LIST_START_CALC:
    "あなたの星を計算しはじめたよ🌌\n" +
    "完了したら一覧が出る。",

  NATAL_LIST_CALC_RUNNING:
    "あなたの星を計算中みたい🌌\n" +
    "完了したら一覧が出る。",

  NATAL_LIST_REPAIRING:
    "あなたの星の情報を整え直してるよ🌌\n" +
    "完了したら一覧が出る。",
  PAID_ONLY:
    "🔵 これは観測ログ＋で表示されるレイヤーです。\n" +
    "現在は案内のみ表示しています。\n" +
    "「＋」で観測ログ＋の説明を見られます。",

  PAID_ONLY_MESSAGES: {
    today_distribution:
      "🔵 ぶんぷは観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_bunpu:
      "🔵 ぶんぷは観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_house:
      "🔵 はうすは観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_tsukiji:
      "🔵 つきじは観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_sorazu:
      "🔵 ソラ図は観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_ura:
      "🌑 うら は補助接点一覧です。\n" +
      "観測ログ＋の探索レイヤーとして開きます。\n" +
      "いまは案内のみ表示しています。",
    paid_seven_day_log:
      "🌌 ななにちログ は観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    paid_seven_day_vision:
      "🌌 ななにちビジョン は観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
    relation:
      "🔵 関係性は観測ログ＋で表示されます。\n" +
      "いまは案内のみ表示しています。\n" +
      "「＋」で観測ログ＋の説明を見られます。",
  },
  RELATION_NO_PAIRS:
    "関係性リストがまだ無いみたい。\n" +
    "相手の登録ができたらここに出るよ。",
  RELATION_PICK_TITLE: "関係性のリストだよ",
  RELATION_PICK_FOOTER: "番号で選んでね。やめるなら「やめる」。",
  RELATION_PICK_INVALID: "番号で選んでね。",
  RELATION_PICK_CANCEL: "キャンセルしたよ。",
  RELATION_PDF_UNAVAILABLE:
    "ただいま準備中です✨️\n" +
    "もう少しで開きます🌟",
  RELATION_REGISTER_OWNER_ONLY: "これはオーナー専用だよ。",
  RELATION_REGISTER_START: "相手の名前を送ってね。",
  RELATION_REGISTER_ASK_DATE: "生年月日を送ってね（例: 1990-01-23）",
  RELATION_REGISTER_ASK_TIME: "出生時刻を送ってね（例: 14:35 / 不明なら「不明」）",
  RELATION_REGISTER_ASK_PLACE: "出生地を送ってね（例: 東京 / 35.68,139.76）",
  RELATION_REGISTER_NAME_INVALID: "名前を送ってね。",
  RELATION_REGISTER_DATE_INVALID: "日付の形式が違うみたい。",
  RELATION_REGISTER_TIME_INVALID: "時刻の形式が違うみたい。",
  RELATION_REGISTER_PLACE_INVALID: "出生地がうまく取得できなかった。",
  RELATION_REGISTER_DONE: "登録完了。計算が終わったら「ふたりの星」で開けるよ。",
  RELATION_REGISTER_CANCEL: "キャンセルしたよ。",
  PLUS_PAUSED:
    "🔵 ソラのこえ＋\n\n" +
    "無料版では、その日の基本の観測を表示しています。\n" +
    "ソラのこえ＋では、そこに分布と深さのレイヤーが加わります。\n\n" +
    "無料版\n" +
    "・その日の基本の観測\n" +
    "・あなたのほし×きょうのそら の基本表示\n" +
    "・見えている共鳴は一部\n\n" +
    "ソラのこえ＋\n" +
    "・ぶんぷ\n" +
    "  その日の接点を、強度の並びで見られます\n" +
    "・あなたのはうす（全ハウス）\n" +
    "  どのハウスに圧が集まっているかを一覧で見られます\n" +
    "・近日の共鳴\n" +
    "  近い日付で強まりやすい接点を先に確認できます\n" +
    "・逆行のレイヤー\n" +
    "  逆行天体の区間をまとめて確認できます\n" +
    "・うら\n" +
    "  あなたのほし×きょうのそら の全共鳴を探索できます\n" +
    "・共鳴の表示量\n" +
    "  無料版より多い接点を見られます",
  PLUS_UNAVAILABLE: "いま入会導線の準備中だよ。",
  PLUS_INVITE: (url) =>
    "🔵 ソラのこえ＋\n\n" +
    "無料版では、その日の基本の観測を表示しています。\n" +
    "ソラのこえ＋では、そこに分布と深さのレイヤーが加わります。\n\n" +
    "無料版\n" +
    "・その日の基本の観測\n" +
    "・あなたのほし×きょうのそら の基本表示\n" +
    "・見えている共鳴は一部\n\n" +
    "ソラのこえ＋\n" +
    "・ぶんぷ\n" +
    "  その日の接点を、強度の並びで見られます\n" +
    "・あなたのはうす（全ハウス）\n" +
    "  どのハウスに圧が集まっているかを一覧で見られます\n" +
    "・近日の共鳴\n" +
    "  近い日付で強まりやすい接点を先に確認できます\n" +
    "・逆行のレイヤー\n" +
    "  逆行天体の区間をまとめて確認できます\n" +
    "・うら\n" +
    "  あなたのほし×きょうのそら の全共鳴を探索できます\n" +
    "・共鳴の表示量\n" +
    "  無料版より多い接点を見られます\n\n" +
    "登録はこちら\n" +
    `${url}`,
  BLUEPRINT_DELAY_NOTICE:
    "🌌 いま設計図の生成が続いています\n\n" +
    "現在、生成リクエストが重なっていて\n" +
    "いつもより時間がかかっています🪐\n\n" +
    "処理は進んでいるので\n" +
    "完成次第、このLINEにお届けします✨️\n\n" +
    "もう少しだけ\n" +
    "このまま待ってもらえると嬉しいです\n\n" +
    "完成すると\n" +
    "あなたの星の構造が見れるようになります🌙",
  PLUS_ALREADY:
    "🔵 ソラのこえ＋ は、朝配信の続きに毎日自動で付いています。\n\n" +
    "見返すときは\n" +
    "・きょう＋（個人の観測ログ）\n" +
    "・そら＋（全体の観測ログ）\n" +
    "で呼び出せます。",

  // ---- blueprint (v25) ----
  BLUEPRINT_NEED_LINE: "この操作はLINEから使ってね🕊️",
  BLUEPRINT_NEED_NATAL:
    "星の設計図（Blueprint v25）は登録済みの人向け🌌\n" +
    "先に「はじめる」で登録してね。",
  BLUEPRINT_NEED_PURCHASE:
    "星の設計図（Blueprint v25）は登録特典だよ🌌\n" +
    "登録済みなら、でき次第このトークにお届けします。",
  BLUEPRINT_FREE_INFO:
    "星の設計図（Blueprint v25）は登録特典だよ🌌\n" +
    "登録済みなら、でき次第このトークにお届けします。",
  BLUEPRINT_ALREADY_PURCHASED:
    "設計図はいつでも開けるよ🌌\n" +
    "「設計図」と送るとURLを返すね。",
  BLUEPRINT_NOT_READY:
    "設計図の準備中だよ🌌\n" +
    "整い次第、このトークにお届けします。",
  BLUEPRINT_PURCHASE_UNAVAILABLE:
    "いま設計図の準備中だよ🌌\n" +
    "整い次第、このトークにお届けします。",
  BLUEPRINT_PURCHASE_READY: (url) =>
    "星の設計図（Blueprint v25）\n" +
    "いま準備中です🌌\n\n" +
    "整い次第、このトークにお届けします。\n" +
    `${url ? `\n${url}\n` : ""}`,
  BLUEPRINT_URL: (url) =>
    "星の設計図（Blueprint v25）はこちら👇\n" +
    `${url}\n\n` +
    "必要なら、また「設計図」で出せるよ。",
  BLUEPRINT_PREPARING_PUSH:
    "🌌 設計図の準備を開始しました。\n" +
    "整い次第、このトークに「設計図を開く」ボタンが届きます。",

});

module.exports = { LINE_COPY };
