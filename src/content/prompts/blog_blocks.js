...
/** 0｜全体圧：俯瞰→重心→ノイズ→余韻（2段落） */
const FLOW_OVERVIEW = `
文章構造（0｜全体圧）:
- 2段落固定（空行1つ）。
- 段落1：FACTSを3文で圧縮（元素数/モード数/逆行/強い角度のうち2つ以上を具体語で）。
- 段落2：重心（どこに寄るか）→ノイズ（迷いやすい点/滞りやすい点）→余韻（結論なし）を2〜3文。
`.trim();

/** 1｜配置（各天体）：事実→作用点→入り方→余韻（2段落まで） */
const FLOW_POSITION_ITEM = `
文章構造（1｜配置 ITEM）:
- 2段落まで（基本2段落、短ければ1段落でも可）。
- 段落1：事実を2文（サイン/度数/ハウス/逆行のうち最低2つ）。
- 段落2：作用点（どこが動くか）→入り方（順番）→余韻（結論なし）を2文。
