...
} = require("../../usecases/reference/calendar_year");
...
function validatePayloads({ reference, deck, exportPayload }) {
  ...
  const checks = [
    { label: "reference", schemaId: "calendar_year.v1.schema.json", data: reference },
    { label: "deck", schemaId: "carousel_deck.v1.schema.json", data: deck },
    { label: "export", schemaId: "calendar_year.export.v1.schema.json", data: exportPayload },
  ];
...
function main() {
  ...
  const templatePath = args.template || path.resolve(process.cwd(), "src/content/templates/instagram/calendar_year.v1.json");
  ...
  const reference = buildCalendarYearReference({ year });
  const template = readJson(templatePath);
  const deck = buildCalendarYearDeck({ reference, template });

  const outDir = path.join(outRoot, String(year), "calendar_year");
  ensureDir(outDir);

  const refPath = path.join(outDir, `${year}.moon_calendar.v1.json`);
  const deckPath = path.join(outDir, `${year}.calendar_year.deck.v1.json`);
  const exportPath = path.join(outDir, `${year}.calendar_year.export.v1.json`);

  const exportPayload = {
    type: "calendar_year_export",
    version: "v1",
    year,
  ...
  if (saveReference) {
    ...
    ensureDir(refDir);
    const savedPath = path.join(refDir, `${year}.moon_calendar.v1.json`);
    writeJson(savedPath, reference);
  }

  console.log("[calendar_year] generated", {
    year,
    outDir,
    reference: refPath,
    deck: deckPath,
