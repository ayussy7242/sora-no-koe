...
function createLineRouter(deps = {}) {
    ...
    // GET /line/debug/story?text=そら&user=U_LOCAL_TEST
    // -------------------- debug: story --------------------
    // 例：
    //  /line/debug/story?text=そら
    //  /line/debug/story?text=きょう&app_user_id=U_TEST
    //  /line/debug/story?text=わたしのほし&app_user_id=U_TEST
    // GET /line/debug/story?text=そら&app_user_id=U_LOCAL_TEST
    router.get("/debug/story", async (req, res) => {
        const d = resolveDeps(req, deps);
        const env = d.env || {};
        const DEBUG_TOKEN = env.DEBUG_TOKEN || null;

        // ✅ debugはtoken必須（既存 debug/routes と同じ安全設計）
        if (!DEBUG_TOKEN) return res.status(500).json({ ok: false, error: "DEBUG_TOKEN is not set" });
        const token = String(req.query.token || "");
        if (!token || token !== String(DEBUG_TOKEN)) return res.status(401).json({ ok: false, error: "unauthorized" });

        try {
            const { db, admin, renderers, geocoder, storyService } = d;

            const text = String(req.query.text || "");
            const cmd = intent.normalizeForCommand(text);
            const intentKey = intent.intentFromCommand(cmd);

            // ✅ 今回仕様：app_user_id に統一（curl側も統一できる）
            ...
            if (!renderers?.renderSoraLine) throw new Error("renderers.renderSoraLine required");
            if (!renderers?.renderSoraAllLine) throw new Error("renderers.renderSoraAllLine required");
            ...
            if (intentKey === intent.INTENT.NATAL) {
                // ✅ debugでも natal list を出す
                out = await natal.handleNatalList({ appUserId });
            } else if (intentKey === intent.INTENT.PUBLIC_SKY) {
                out = await story.buildSky(); // public固定
            } else if (intentKey === intent.INTENT.PERSONAL_TODAY) {
                // ✅ app_user_id が public のときは guide に寄せる（普段の挙動）
                if (!appUserId || appUserId === "public") {
                    out = await story.buildSkyWithGuide();
                } else {
                    out = await story.buildToday({ appUserId });
                }
            } else {
                out = { text: story.renderFallback() || "unknown intent" };
            }
