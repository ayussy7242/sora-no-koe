 ...
 * - 「今日」or 日付で storyService → renderers.renderLine
 * - 「はじめる」導線：ネイタル収集ステート（生年月日→出生時刻→出生地→geo）
 * - geo: engine/geocode.js に統合（cache / direct lat,lon OK）
 *
 * 最適化:
 * - env 参照の順序バグ（env is not defined）完全排除
 * - 生年月日: 1990-07-24 / 1990/07/24 / 19900724 対応
 * - 地名 geocode: 変換クエリ生成 + リトライ
 * - 失敗理由を Cloud Run logs に必ず出す（REQUEST_DENIED等の原因追跡）
 */
