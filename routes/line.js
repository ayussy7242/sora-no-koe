...
function createLineRouter(deps = {}) {
    ...
    async function enqueueNatalCalcJob(appUserId) {
        ...
        // 最低限：date + time が無いと ASC/MC 計算できない
        const date_local = b.date_local || null;
        const time_hm = b.time_hm || null;
        const timezone = b.timezone || DEFAULT_TZ;

        // place は lat/lon があれば最高。無いなら worker が houses を作れない
        const lat = typeof b.lat === "number" ? b.lat : null;
        const lon = typeof b.lon === "number" ? b.lon : null;

        const place_text = b.place_text || null;
        const place_formatted = b.place_formatted || null;
        ...
        const job = {
            ...
            line_user_id: null, // あってもいいけど必須じゃない
            birth: {
                date_local,
                time_hm,
                timezone,
        ...
        // 既存の queued/running が残ってたら増殖するので、ジョブは “固定ID” にすると強い
        // 例: {appUserId} をdocIdにする（常に上書き）
        await db.collection("jobs_natal_calc").doc(appUserId).set(job, { merge: true });
    }

    // --------------------
    // FIX: command intents (normalized input assumed)
    // --------------------
    function intentFromCommand(normalizedCmdText) {
        const t = String(normalizedCmdText || "").trim();
        ...
        try {
            ...
            for (const event of events) {
                ...
                if (event?.type === "message" && event?.message?.type === "text" && replyToken) {
                    ...
                    // reset (最初から もここに混ぜる)
                    if (/^(リセット|reset|最初から|はじめから|最初からやり直す|はじめからやり直す)$/i.test(cmdText)) {
